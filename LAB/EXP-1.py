# Demonstration of Regular Expressions in Python
import re

# Sample text
text = "My phone number is 9876543210 and my email is example@gmail.com."

# 1. Match: Checks if the pattern is at the beginning of the string
match_result = re.match(r"My", text)

if match_result:
    print("Match found at the beginning:", match_result.group())
else:
    print("No match found at the beginning.")

# 2. Search: Finds the first occurrence of the pattern anywhere in the string
search_result = re.search(r"\d{10}", text)

if search_result:
    print("Phone number found:", search_result.group())
else:
    print("Phone number not found.")

# 3. Search for email
email_result = re.search(r"\S+@\S+\.\S+", text)

if email_result:
    print("Email found:", email_result.group())
else:
    print("Email not found.")